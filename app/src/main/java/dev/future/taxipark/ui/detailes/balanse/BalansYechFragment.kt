package dev.future.taxipark.ui.detailes.balanse
...
import dev.future.taxipark.databinding.FragmentBalansYechBinding
import dev.future.taxipark.databinding.LimitCommisionDialogBinding
...
//                }
    ...
    private fun moneyTaking() = with(binding) {
        viewModel.monekTaking(SaveUserInformation.getAuthInfo().authKey.toString())
            .observe(viewLifecycleOwner) { status ->
                when (status.status) {
                    Status.SUCCESS -> status.data.let { response ->
                        binding.swipe.isRefreshing = false
